...
void* pmalloc(int size) {
  ...
  if (size != NODESIZE)
    return malloc(size);
  if (nodesleft == 0) {
    freenode = static_cast<char*>(malloc(NODEGROUP*NODESIZE));
    nodesleft = NODEGROUP;
  }
  nodesleft--;
  p = static_cast<void*>(freenode);
  freenode += NODESIZE;
  return p;
}
...
void initbins(int maxelms, int pmaxval) {
  ...
  bins = maxelms;
  maxval = pmaxval;
  bin = static_cast<struct node**>(pmalloc(bins*sizeof(struct node *)));
  sentinel = static_cast<struct node*>(pmalloc(sizeof(struct node)));
  sentinel->val = maxval;
  for (i = 0; i < bins; i++)
    bin[i] = sentinel;
  bincnt = 0;
...
struct node *rinsert(struct node *p, int t) {
  if (p->val < t) {
    ...
    struct node *q = static_cast<struct node*>(pmalloc(sizeof(struct node)));
    q->val = t;
    q->next = p;
    p = q;
    bincnt++;
